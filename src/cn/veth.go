...
// and brings the host end up. Returns a CIDR of the container network and an error.
func CreateVethPair(ctx context.Context, containerID string, namespacePID int) (string, error) {
	err := ctx.Err()
	if err != nil {
	...
	vethALink, ipAndCIDRstring, err := setupVethLink(ctx, vethA, vethB, namespacePID)
	if err != nil {
...
// Returns the interface, ipAndCIDR as string, and an error.
func setupVethLink(ctx context.Context, vethA, vethB string, namespacePID int) (netlink.Link, string, error) {
	err := ctx.Err()
	if err != nil {
		return nil, "", fmt.Errorf("context cancellation: %w", err)
	}
	...
	ipAndCIDRString := AllocateIP()
	addr, err := netlink.ParseAddr(ipAndCIDRString)
	if err != nil {
	...
	return vethALink, ipAndCIDRString, nil
}
